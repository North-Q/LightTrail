"""LLM 客户端实现：OpenAI 兼容接口的 async-first 薄封装（B3-1 重写；B3-5 迁入适配层）。

归属：本文件是 LLM 客户端的**实现真源**（适配层 L4）。旧路径 `lighttrail.llm.client`
保留为 re-export shim 供迁移期调用方使用；TODO(B5-4): 域工具改经 ToolContext.llm 端口后删除 shim。
