...
**版本**：v2.0.1 ｜ **作者**：高见远（Gao，软件架构）｜ **v2.0 修订**：架构通 ｜ **对应 PRD**：v0.3
**代码基线**：**E1–E5 已实现**（20 commits）——src+tests ~7900 行，13 个已注册工具，pytest 149 全绿、ruff 0 告警、smoke 19 项通过

> **v2.0.1 变更摘要**（2026-09-07）：代码基线从「~2300 行/43 测试」更新为「E1–E5 全部落地（149 测试）」；§2.3 / §2.9 补注 **ADR-002 平台中立性重构**已落地（并发策略可配置 `LLM_SERIAL_LLM`、能力矩阵可注入、`LLM_` 前缀配置、移除「深推理与工具互斥」的平台假设——模型名只出现在 config 默认值/.env/能力矩阵）。架构主体设计不变。
>
> **v2.0 变更摘要**：
