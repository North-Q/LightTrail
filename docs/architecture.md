...
**版本**：v2.0 ｜ **作者**：高见远（Gao，软件架构）｜ **v2.0 修订**：架构通 ｜ **对应 PRD**：v0.2
**代码基线**：~2300 行（Agent ReAct 循环 + 工具注册表 + 12 个已注册工具 + 串行锁 + 28 项测试全绿）

> **v2.0 变更摘要**：
> 1. 交互层从「仅 CLI」升级为 **CLI + FastAPI Web 服务 + Web UI**，新增 SSE 流式协议设计（§2.8）；
> 2. 新增**并发与会话模型**：把 ECNU 串行约束从限制转化为架构简化红利（§2.9）；
...
flowchart TB
    ...
    subgraph L6["⑥ 基础设施层 Infra"]
        CLIENT["ChatClient（已实现）<br/>串行锁 / 重试 / 超时"]
...
**边界设计的核心原则**：领域智能（摄影知识）沉淀在②④层，通用智能（推理与对话）收敛在③层，工程保障（锁/重试/追踪/配额/评估）沉淀在⑥层。这让"换 LLM 平台"只动⑥，"加摄影场景"只动②④，"换交互形态"只动①——CLI 到 Web 的演进将零改动②–⑥层，这是对边界设计最直接的验证。
...
**PRD 映射**：D2"机位×天象匹配、赶场调度"（固定步骤序列）、M2"建议依据、推理可见"、非功能"串行调用/配额有限"。

---

### 2.3 双模型路由：能力边界驱动 + 配额感知

**决策**：`ModelRouter` 按**能力需求**而非"任务大小"路由：
- 需要**工具调用或图片理解** → 强制 `ecnu-plus`（唯一具备该能力的模型）；
- 需要**长上下文深度综合、纯推理**（方案综合、复盘处方生成）→ `ecnu-max`（1M 上下文 + reasoning_effort，且不接受工具/图片——输入已被管线预处理为结构化文本，恰好绕开其短板）；
- 默认对话 → `ecnu-plus`。

路由决策由**调用方声明意图**（`needs_tools` / `needs_vision` / `needs_deep_reasoning`）+ Router 校验能力矩阵完成，而不是让 Router 猜测。规则表硬编码、可单测。

**v2.0 增补：QuotaLedger 配额账本**。ECNU 平台配额为「每 5 小时 2000 / 每天 5000 / 每月 50000 credits」，且 plus 与 max 计价不同（输入分缓存命中/未命中）。路由之前先过账本：

1. **记账**：每次调用按 token 实际消耗入账，滚动窗口统计（5h/日/月）；
2. **预估**：管线入口按步骤数 × 历史均值预估本趟成本，超预算时在管线入口就拒绝或降级，而不是跑到一半断粮；
3. **降级链**：配额水位 >90% 时，max 综合自动降级为 plus 综合 + thinking（质量略降、成本约 1/3），并在 DecisionCard 中标注"降级原因"——降级本身也是可解释性的一部分。

**候选对比**：统一只用 plus（浪费 max 的 1M 上下文与推理深度，且 plus 的 256K 在长轨迹注入下有压力）vs 动态 LLM 路由（让小模型判断该用谁——引入第三个不可控环节，平台只有两款模型，规则路由已穷尽）。**选择理由**：两款模型的能力集**互补且不重叠**（plus 有工具/视觉，max 有长上下文/深推理），这是"能力边界驱动架构"的教科书案例：架构上把系统切成"感知执行段（plus）"与"综合推理段（max）"，中间用结构化文本交接，正好同时避开 max 不能调工具、plus 上下文较短的各自短板。配额层面也获益：把最贵的综合调用收敛到管线末端唯一一次，再由账本兜底极端情况。

**PRD 映射**：A3 模型路由 + LLM 平台约束（plus/max 能力差异、串行、配额）。
...
| 复合 | 内部聚合多数据源，输出融合评分 | 单工具 ≤10s 超时预算内完成全部子请求；子源失败要降级（返回部分结果 + 标注缺失），不整体报错 | 天气+云量+湿度 → 火烧云评分；机位×天象匹配 | 把"多源聚合"这一固定模式收进工具内部，Agent 循环只看到一次调用——减少 ReAct 轮次、降低 token 与串行锁占用 |
| 智能 | 工具内部自建 `ChatClient` 调多模态 LLM | 内部 LLM 调用**不允许再触发工具**（深度=1，防递归）；必须走同一个串行锁与配额账本；输出必须结构化（JSON Schema 约束） | 照片多模态分析（EXIF+画面 → 技术诊断） | "LLM-in-a-tool" 让 Agent 把多模态分析当作普通工具调用，主循环零改动；深度限制为 1 是防失控的关键红线 |
...
② 行为准则（静态，~300 tok）        —— 串行约束、置信度表达规范、不确定性坦白原则
...
```
event: queued     —— 已进入串行队列（见 §2.9），附排队位置
...
### 2.9 【新增】并发与会话模型：把串行约束转化为架构红利

**问题**：现有 `threading.Lock` 串行锁只保护单进程，且同步锁在 FastAPI 的 async 事件循环里会阻塞所有请求。Web 化后必须正面回答：多会话并发时串行约束怎么保证？

**决策**：三层模型——

1. **执行层：`asyncio.Semaphore(1)` 全局 LLM 队列**。所有 LLM 调用（含智能工具内部调用）统一经 `ChatClient.acall()` 排队获取信号量。ecnu-plus/max 的调用在信号量内执行，工具计算、HTTP 数据获取（天气/地图等非 LLM 请求）在信号量外可并发——**串行的只是 LLM，不是整个系统**。这既满足平台"避免并行请求 LLM"的约束，又不让慢天气 API 堵住别人的对话。
2. **会话层：`SessionManager`**。`session_id → {消息历史, PipelineContext, 记忆工作区}`，进程内字典 + JSON 落盘（A-08 对话持久化一并兑现）。多会话共享 LLM 队列，各自独立的上下文与记忆命名空间。单机自用场景不需要认证体系，预留 `user_id` 字段即可。
3. **体验层：排队显式化**。请求入队即推 `queued` 事件（附位置），用户看到的是"排队第 1 位 · 预计 8s"而不是转圈。串行从缺陷变成可解释的体验。

**架构红利叙事**：ECNU 的串行+配额约束意味着**水平扩展零收益**——十个 worker 也只有一个能调 LLM，反而引入分布式锁问题。所以最优部署形态就是**单进程 uvicorn**：无分布式状态、无负载均衡、无会话粘连问题，一台小服务器跑全栈。这是"约束驱动架构"的第二个案例（第一个是 §2.3 的双模型切分）：别人眼中的平台限制，恰好免掉了 Web 系统最复杂的可扩展性设计。若未来平台放开并发，`Semaphore(1)` 改一个数字即完成升级——架构已预留，成本是一行代码。

**PRD 映射**：NF-02 串行调用约束、A-08 对话持久化、NF-01 响应时间。
...
| 层 | 对象 | 方法 | 现状/计划 |
|---|---|---|---|
| L1 工具层 | 纯计算/复合工具 | pytest 精确断言（NPF 公式、500 法则边界、ND 档位换算） | ✅ 已实现，28 用例全绿 |
...
sequenceDiagram
    ...
    P->>TR: dispatch 数据采集（LLM 队列外，数据请求可并发）
...
| **水平扩展 / 分布式部署** | 平台串行+配额约束下扩展零收益，单进程 uvicorn 即最优（见 §2.9） | 平台放开并发限制且用户量真实增长时（`Semaphore(1)` 改一个数字起步） |
| **并行 LLM 调用** | 平台明确建议串行，配额有限；单工具 10s 超时下串行总时长可预算 | 平台放开并发限制、且赶场调度等场景的延迟成为真实瓶颈时 |
...
|---|---|---|---|
| **E1 地基拆分** | `agent/core.py` 拆出：`agent/loop.py`（ReAct 循环）、`agent/context.py`（ContextBuilder）、`llm/router.py`（ModelRouter） | `agent/{core,loop,context}.py`、`llm/router.py` | `Agent.run()` 委托给 loop，公开 API 不变；28 用例不动 |
...
| **E7 Web 层** | `ChatClient` 增加 async 通道（`acall` + Semaphore(1) 队列）、`api/app.py`（FastAPI + SSE 路由）、`api/session.py`（SessionManager）、`web/`（SPA 工程化，原型组件化）；trace 事件桥接 SSE | `llm/client.py`、`api/{app,session,routes}.py`、`web/` | CLI 路径完全不变；同步 ChatClient 保留为 async 的薄封装 |
...
| Router 类（GPT-Router 等） | `llm/router.py` 能力矩阵 + QuotaLedger | 通用 router 按"成本/难度"猜测路由；我们按**能力声明**确定性路由——两款模型能力互补不重叠，规则即穷举，无需学习；配额账本是平台约束驱动的增量 |
...
**自研 vs 套框架的一句话总结**：框架的价值在通用场景的生态与现成组件，代价是抽象税与黑盒；LightTrail 的场景是垂直、流程已知、平台约束明确（串行/配额/双模型能力互补），自研轻量骨架在可控性、可解释性、可测试性上全面占优，且基线代码已证明核心机制并不复杂——**架构的复杂度应该花在记忆、可解释性、上下文工程、评估体系这些真正决定 Agent 质量的地方，而不是花在框架本身**。

---

## 附：设计原则回顾

1. **能力边界驱动架构**：plus/max 能力互补 → 感知执行段与综合推理段分离；
2. **约束驱动简化**：串行+配额约束 → 单进程部署、显式排队、配额账本——限制被转化为架构红利而非包袱；
