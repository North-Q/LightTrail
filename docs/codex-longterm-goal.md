...
```
📍 当前位置：架构重建起点（B0 止血护栏之前）
  代码基线：main 分支 HEAD 2129d62 ｜ pytest 223 全绿 ｜ ruff 0 ｜ 15 工具
  文档基线：v4 方案定稿（D1–D14 全部拍板）｜ REFACTOR-ROADMAP v1.0 已就位
🎯 当前目标：B0 止血护栏（B0-1 会话三 bug → B0-2 tokens → B0-3 假注释 → B0-4 收口）
...
```mermaid
graph TD
    S([📍当前位置<br/>B0 之前 / HEAD 2129d62<br/>223 测试全绿 / v4 方案定稿]) --> B0
