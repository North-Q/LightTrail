...
| 阶段 | 对应架构 | 主题 | 目标 |
|------|----------|------|------|
| 阶段一 | E1+E2 | 地基拆分与可观测性 | `agent/core.py` 拆出 loop/context/router；TraceRecorder 贯穿三层 |
| 阶段二 | E3+E4 | 记忆层与配额感知 | 四层记忆注入 + reason 通道 + QuotaLedger 配额账本 |
| 阶段三 | E5 | 决策编排与输出契约 | 四管线编排 + Intent/DecisionCard pydantic 契约 + 自愈重试 |
| 阶段四 | E6 | 多模态与差异化 | 照片分析（智能工具）+ 反推方案 + 事件/语义记忆 |
| 阶段五 | E7 | Web 服务层 | async ChatClient + FastAPI + SSE 流式 + SPA 前端（trace 即 UI） |
| 阶段六 | E8 | 评估体系 | 三层评估：黄金用例集 + LLM-as-judge，质量回归门禁 |
| 阶段七 | — | 开源发布 | 文档完善、贡献指南、版本发布 |
...
| M1 个性化记忆（档案/事件/语义） | ⏳ 未开始 | E3-1 ~ E3-3 |
| M2 可解释性（轨迹/依据/来源/推理） | ⏳ 未开始 | E2-2（TraceRecorder）+ E7-4（SSE 桥接）+ E5-2（契约） |
| A-06 模型路由 + 配额 | ⏳ 未开始 | E4-1（ModelRouter）+ E4-2（QuotaLedger） |
| D1.1 一句话出方案 / D2.3 计划生成 | ⏳ 未开始 | E5-1（四管线编排） |
| D1.2 照片反推 / D4 照片分析 | ⏳ 未开始 | E6-1 / E6-2 |
| D2.2 多机位赶场调度 | ⏳ 待定 | 见 §4 待明确事项 |
| A-08 对话持久化 / Web UI | ⏳ 未开始 | E7-2（SessionManager）/ E7-3、E7-5 |
| 非功能：可测试性 / 质量回归 | ⏳ 未开始 | E8-1 / E8-2 |
...
> 阶段目标：D4 照片分析（智能工具，深度=1 红线）、D1.2 照片反推、语义记忆提炼。架构 v2.0 §2.4「智能工具」约束是本阶段硬红线。
...
- **验收标准**：mock ChatClient 验证消息含 `image_url`；图片缩放后 base64 体积 < 上下文限制；无 EXIF 时 `exif` 空 dict 不阻塞
...
- **实现步骤**：规则表（如「同题材成功率 ≥70% 且样本 ≥3 → 语义结论」）；写入前 double-confirm 防污染
